...
for imagePath in imagePaths:
    ...
    # load the image, convert it to RGB channel ordering, and resize it to be a fixed 224x224 pixels while ignoring aspect ratio
    ...
    # convert the datat and the labels to Numpy arrays while scaling the pixel
...
try:
    ...
    print(f"Error drawing val_loss: {e}")

plt.plot(np.arange(0, N), H.history["accuracy"], label="train_acc")
try:
    plt.plot(np.arange(0, N), H.history["val_accuracy"], label="val_acc")
except Exception as e:
    print(f"Error drawing val_acc: {e}")
